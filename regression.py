...
def fit_regression_model(X, y):
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        if epoch % 100 == 0:
            print(f'Epoch {epoch}/{num_epochs}, Loss: {loss.item()}') # This is a good place to print the loss every 1000 epochs.
        if abs(previos_loss - loss.item()) < 0.001:
            print ("loss is not change significantly, process stopped early.")
            break
